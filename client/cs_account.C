...
///
/// \return Zero on success, ERR_FOPEN if creating the temporary file failed,
///         ERR_RENAME if renaming the temporary file failed.
...
///
/// \return Zero on success, ERR_FOPEN if the account file could not
///         be opened, ERR_XML_PARSE if the XML is malformed.
int PROJECT::parse_account_file_venue() {
    ...
    std::string path = get_account_filename(master_url);
    FILE* in = boinc_fopen(path.c_str(), "r");
    if (!in) {
        return ERR_FOPEN;
    }
    ...
    while (fgets(buf, 256, in)) {
        ...
        if (match_tag(buf, "</venue>")) {
            ...
            retval = copy_element_contents(in, "</project_specific>", project_specific_prefs);
            if (retval) {
                return retval;
            }
            continue;
...
int CLIENT_STATE::parse_account_files() {
    ...
    while (dir.scan(name)) {
        ...
        int retval = project->parse_account(f);
        fclose(f);
        if (retval) {
...
int CLIENT_STATE::add_project(const std::string& master_url, const char* _auth, const char* project_name, bool attached_via_acct_mgr) {
    ...
    int retval = project->write_account_file();
    if (retval) return retval;

    std::string af_name = get_account_filename(canonical_master_url);
    FILE* f = boinc_fopen(af_name.c_str(), "r");
    if (!f) {
        return ERR_FOPEN;
    }
